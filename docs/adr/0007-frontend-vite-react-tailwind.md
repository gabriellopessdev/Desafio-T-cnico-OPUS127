...
Aplicação em `frontend/` (Vite + React + TypeScript + Tailwind; código em `frontend/src`). Uma tela. Três GET em paralelo para as 3 últimas semanas epidemiológicas **fechadas** (ver [ADR 0008](0008-painel-semanas-fechadas.md)). Cards (cor por nível 1–4) + tabela + gráfico Recharts. `VITE_API_URL`. Sem roteador além do necessário, sem auth.
